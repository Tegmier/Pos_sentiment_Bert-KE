...
from model.model_MFAN import FinetuneBertMFAN, bert_model
...
train_test_rate = 0.7
decay_rate = 0.15
model_name = "main_lab_distributed_amp_dmw_FAN_logger"
#############################################################################
...
def train(rank, world_size, data, logger):
    ...
    # 初始化模型
    model = FinetuneBertMFAN(bert_model=bert_model, y_dim=y_dim, z_dim=z_dim, embedding_dim=embedding_dim).cuda(rank)
...
def eval(data, logger):
    model = FinetuneBertMFAN(bert_model=bert_model, y_dim=y_dim, z_dim=z_dim, embedding_dim=embedding_dim).cuda()
