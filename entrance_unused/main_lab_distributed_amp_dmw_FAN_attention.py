...
# from utils.logging import get_logger, write_log
...
def train(rank, world_size, data):
    ...
    # 初始化模型
    model = FinetuneBertFANAttention(bert_model=bert_model, y_dim=y_dim, z_dim=z_dim, embedding_dim=embedding_dim).cuda(rank)
    ...
    for epoch in range(nepochs):    
        ...
        for inputs in data_loader:
            with autocast(device_type='cuda'):
                ...
                y = y.reshape(-1)
                z = z.reshape(-1)
                loss_y = criterion(y_pred, y) * attention.reshape(-1)
                loss_z = criterion(z_pred, z) * attention.reshape(-1)
                loss_y = loss_y.mean()
                loss_z = loss_z.mean()
