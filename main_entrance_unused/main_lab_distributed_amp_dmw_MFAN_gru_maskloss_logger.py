...
from model.model_MFAN_gru import FinetuneBertMFANgru, bert_model
...
train_test_rate = 0.7
decay_rate = 0.8
model_name = "main_lab_distributed_amp_dmw_MFAN_gru_maskloss_logger"
#############################################################################
...
def train(rank, world_size, data, logger):
    ...
    # 初始化模型
    model = FinetuneBertMFANgru(bert_model=bert_model, y_dim=y_dim, z_dim=z_dim, embedding_dim=embedding_dim).cuda(rank)
...
def eval(data, logger):
    model = FinetuneBertMFANgru(bert_model=bert_model, y_dim=y_dim, z_dim=z_dim, embedding_dim=embedding_dim).cuda()
