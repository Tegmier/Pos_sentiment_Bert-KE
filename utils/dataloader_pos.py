...
import torch
from config.bert_model_config import tokenizer
import nltk
...
labels2pos = {'PRON': 0, 'ADV': 1, 'VERB': 2, 'ADP': 3, 'DET': 4, 'ADJ': 5, 'NUM': 6, 'CONJ': 7}
...
def batch_padding_tokenizing_collate_function(batch):
    ...
    encoded_tweet = tokenizer.batch_encode_plus(
            ...
            add_special_tokens=False,  # 添加特殊token，如[CLS]和[SEP]
            padding=True,  # 填充到最大长度
            truncation=True,  # 截断超过最大长度的句子
            return_tensors='pt'  # 返回PyTorch张量
        )
    tweet_tokenization = [tokenizer.convert_ids_to_tokens(input_ids) for input_ids in encoded_tweet['input_ids']]
    tweet_pos = []
    for tweet in tweet_tokenization:
        print(tweet)
        pos_tags = pos_tag(tweet, tagset='universal')
        print(pos_tags)
    tag_tensor = []
    for t in tags:
